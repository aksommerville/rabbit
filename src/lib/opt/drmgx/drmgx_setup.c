...
int drmgx_open_file(struct drmgx *drmgx) {
  ...
  const char *device_path=drmgx->reqdevice;
  if (!device_path) device_path="/dev/dri/card0";
  
  if ((drmgx->fd=open(device_path,O_RDWR))<0) {
    fprintf(stderr,"%s: %m\n",device_path);
    return -1;
  }
...
static int drmgx_pick_mode(
  ...
  if ((amode->vrefresh==drmgx->reqrate)&&(bmode->vrefresh!=drmgx->reqrate)) return -1;
  if ((amode->vrefresh!=drmgx->reqrate)&&(bmode->vrefresh==drmgx->reqrate)) return 1;
  
  // If one is smaller than the client's framebuffer, we don't want it.
  int asmall=((amode->hdisplay<drmgx->fbw)||(amode->vdisplay<drmgx->fbh));
  int bsmall=((bmode->hdisplay<drmgx->fbw)||(bmode->vdisplay<drmgx->fbh));
...
static int drmgx_select_best_mode(struct drmgx *drmgx,drmModeResPtr res) {
  //fprintf(stderr,"%s...\n",__func__);
  ...
  for (;conni<res->count_connectors;conni++) {
    ...
    for (;modei<conn->count_modes;modei++,mode++) {
      ...
      if (drmgx_pick_mode(drmgx,bestconn,&bestmode,conn,mode)>0) {
  ...
  fprintf(stderr,
    "drmgx: Selected video mode '%.*s' (%dx%d@%dHz)\n",
...
int drmgx_configure(struct drmgx *drmgx) {
  ...
  if (drmgx_select_best_mode(drmgx,res)<0) {
 ...
 * This only creates a context. The more specific stuff like shaders belongs to our "render" unit.
 */
