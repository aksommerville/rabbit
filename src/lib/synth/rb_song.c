...
#if 0//XXX first attempt, just a MIDI dechunker...

/* Decode MThd chunk.
 */
 
static int rb_song_decode_MThd(struct rb_song *song,const uint8_t *src,int srcc) {
  if (song->division) return -1; // Multiple MThd
  if (srcc<6) return -1; // Short MThd
  
  song->format=(src[0]<<8)|src[1];
  song->track_count=(src[2]<<8)|src[3];
  song->division=(src[4]<<8)|src[5];
  
  if (song->format>1) return -1; // Single track or simultaneous multitrack only
  if (song->division&0x8000) return -1; // SMPTE timing not supported
  if (!song->division) return -1; // Invalid division (and we use this for MThd detection)
...
static int rb_song_decode_MTrk(struct rb_song *song,const uint8_t *src,int srcc) {
  if (song->trackc>=song->tracka) {
    int na=song->tracka+8;
    if (na>INT_MAX/sizeof(struct rb_song_track)) return -1;
    void *nv=realloc(song->trackv,sizeof(struct rb_song_track)*na);
    if (!nv) return -1;
    song->trackv=nv;
    song->tracka=na;
  }
  struct rb_song_track *track=song->trackv+song->trackc++;
  track->v=src;
  track->c=srcc;
  return 0;
}

/* Unknown chunk -- Could log them or fail, I'm choosing to ignore them.
 */
 
static int rb_song_decode_unknown(struct rb_song *song,uint32_t chunkid,const uint8_t *src,int srcc) {
  return 0;
}

/* Assertion completion of decode.
 */
 
static int rb_song_decode_finish(struct rb_song *song) {
  if (!song->division) return -1; // No MThd
  if (song->trackc<1) return -1; // No MTrk
  return 0;
}
...
static int rb_song_decode(struct rb_song *song) {
  int srcp=0;
  int stopp=song->storagec-8;
  while (srcp<=stopp) {
    const uint8_t *src=song->storage+srcp;
    int chunkid=(src[0]<<24)|(src[1]<<16)|(src[2]<<8)|src[3];
    int len=(src[4]<<24)|(src[5]<<16)|(src[6]<<8)|src[7];
    srcp+=8;
    if ((len<0)||(srcp>song->storagec-len)) return -1;
    const uint8_t *chunk=src+8;
    srcp+=len;
    switch (chunkid) {
      case RB_CHUNKID_MThd: if (rb_song_decode_MThd(song,chunk,len)<0) return -1; break;
      case RB_CHUNKID_MTrk: if (rb_song_decode_MTrk(song,chunk,len)<0) return -1; break;
      default: if (rb_song_decode_unknown(song,chunkid,chunk,len)<0) return -1;
    }
  }
  return rb_song_decode_finish(song);
}
...
struct rb_song *rb_song_new(const void *src,int srcc) {
  ...
  if (!(song->storage=malloc(srcc))) {
    rb_song_del(song);
    return 0;
  }
  memcpy(song->storage,src,srcc);
  song->storagec=srcc;
  
  if (rb_song_decode(song)<0) {
    rb_song_del(song);
    return 0;
  }
...
void rb_song_del(struct rb_song *song) {
  ...
  if (song->storage) free(song->storage);
  free(song);
}
