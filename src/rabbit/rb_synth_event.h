...
#if 0//XXX this is a truer MIDI setup -- we can simplify and front-load a lot of processing...
struct rb_song {
  int refc;
  uint16_t format;
  uint16_t track_count; // from MThd, not necessarily the count of MTrks (and not useful either)
  uint16_t division;
  struct rb_song_track {
    const uint8_t *v; // points into (storage)
    int c;
  } *trackv;
  int trackc,tracka;
  uint8_t *storage;
...
struct rb_song_player {
  ...
  //TODO tempo
  int trackc;
  struct rb_song_player_track {
    const uint8_t *v; // borrowed from (song)
    int c;
    int have_notes; // nonzero if we received at least one Note On
    int p;
    int delay; // frames; <0 if unknown, must read from stream
    uint8_t rstat;
    // "0" are the state at the repeat point:
    int p0;
    int delay0;
    uint8_t rstat0;
  } trackv[];
